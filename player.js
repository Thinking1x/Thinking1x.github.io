...
var colorHue = 0;
var visualizerAvailable = true; // becomes false if audio graph setup fails; playback still works
...
// Track the raw (untouched) URL for the currently loading track, so we can
// retry with it if the "cleaned" URL turns out to need the auth token after all.
var lastRawTrackUrl = "";
var hasRetriedWithRawUrl = false;
...
async function loadTrack(i, autoplay = false) {
    ...
    // 2. CLEAN THE URL
    // NOTE: we keep the raw (untouched) URL around too, because some storage
    // backends require the auth token in the query string. If the cleaned
    // URL fails to load, the 'error' handler below will retry with the raw one.
    const rawUrl = track.file || "";
    let cleanUrl = rawUrl;
    if (cleanUrl.includes('&jwt=')) cleanUrl = cleanUrl.split('&jwt=')[0];
    if (cleanUrl.includes('?jwt=')) cleanUrl = cleanUrl.split('?jwt=')[0];

    lastRawTrackUrl = rawUrl;
    hasRetriedWithRawUrl = false;
        ...
        try {
            ...
            if (playPromise !== undefined) {
                playPromise.catch(error => {
                    // Ignore AbortError (happens when the user quickly skips tracks) —
                    // anything else means playback genuinely failed to start.
                    if (error.name !== 'AbortError') {
                        console.warn('Playback failed to start:', error.name, error.message);
                        if (playIcon) playIcon.className = 'fas fa-play';
                    }
                });
            }
        } catch (e) {
            console.warn('Playback threw synchronously:', e);
            if (playIcon) playIcon.className = 'fas fa-play';
        }
    } else {
        if (playIcon) playIcon.className = 'fas fa-play';
...
function togglePlay() {
    ...
    if (audioEl.paused) {
        ...
        if (playPromise !== undefined) {
            playPromise.catch(e => {
                if (e.name !== 'AbortError') console.warn('Play interrupted:', e.name, e.message);
            });
        }
    } else {
        audioEl.pause();
...
function nextTrack(isAutoAdvance = false) {
    ...
    if (repeatMode === 2 && isAutoAdvance && audioEl) {
        ...
        audioEl.play().catch(e => {
            if (e.name !== 'AbortError') console.warn('Repeat-one replay failed:', e.name, e.message);
        });
        return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (audioEl) {
        ...
        // ---- FIX: this was completely missing. Without it, any load/network
        // failure (bad URL, missing auth token, 404, CORS block, etc.) failed
        // silently — the icon would just spin forever with no way to diagnose it.
        audioEl.addEventListener('error', () => {
            ...
            console.error(
                ...
                err ? (codeNames[err.code] || `code ${err.code}`) : 'unknown error',
                'src:', audioEl.currentSrc
            );

            // If we stripped an auth token off the URL and haven't tried the
            // original yet, retry once with the raw (untouched) URL — the
            // stream may require that token.
            if (!hasRetriedWithRawUrl && lastRawTrackUrl && lastRawTrackUrl !== audioEl.src) {
                hasRetriedWithRawUrl = true;
                console.warn('Retrying with original (untouched) track URL...');
                audioEl.src = lastRawTrackUrl;
                audioEl.load();
                audioEl.play().catch(e => {
                    if (e.name !== 'AbortError') {
                        console.error('Retry with raw URL also failed:', e.name, e.message);
                        const playIcon = document.getElementById('playIcon');
                        if (playIcon) playIcon.className = 'fas fa-play';
                    }
                });
            ...
            // Give up: reset the icon instead of leaving it stuck spinning forever.
            const playIcon = document.getElementById('playIcon');
            if (playIcon) playIcon.className = 'fas fa-play';
        });
    }
    ...
    if (volumebarEl && audioEl) {
        // Restore any previously saved volume
        try {
            ...
            if (savedVolume !== null && !isNaN(savedVolume)) {
                ...
                volumebarEl.value = Math.round(audioEl.volume * 100);
            }
        } catch (e) {
            console.warn('Could not restore saved volume:', e);
        }
...
function setupVisualizer() {
    ...
    // FIX: everything below can throw (autoplay-policy restrictions, the audio
    // element already being connected elsewhere, a CORS-tainted source, etc.).
    // Previously an exception here could leave audioCtx half-initialized and
    // permanently block retries (because of the `if (audioCtx) return;` guard),
    // and in the worst case could interfere with normal playback. Now a failure
    // here just disables the visualizer and lets audio play through the normal
    // HTML5 <audio> path instead.
    try {
        ...
        console.warn('Visualizer setup failed — continuing with plain audio playback:', e);
        visualizerAvailable = false;
        audioCtx = null;
        analyser = null;
        dataArray = null;
    ...
    try {
        ...
        // Canvas setup failing shouldn't take audio down with it.
        console.warn('Visualizer canvas setup failed:', e);
    }
}
...
function startVisualizer() {
    if (!visualizerAvailable || !analyser) return;
...
function renderFrame() {
    ...
    if (!isVisualizerRunning || !audioEl || audioEl.paused || !analyser) {
